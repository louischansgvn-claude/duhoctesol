<?php
/**
 * Other-countries (THPT + Đại học + Anh ngữ) seeder for non-production.
 *
 * Imports the bundled dataset (inc/data/other-countries/other-countries-import.json)
 * into the `school` CPT as public-clean SEO articles, tagged with the country term
 * + study_level=thpt|dai-hoc|anh-ngu. Featured image = the school logo pre-uploaded
 * to /wp-content/uploads/other-countries/<file>.
 *
 * FULLY ISOLATED from the US and Canada/Australia import subsystems:
 *   - identity meta is `_duy_othc_id` (never `_duy_ps_id` / `_duy_hs_school_id` /
 *     `_duy_caau_id`), so those seeders' trash sweeps can never touch these posts,
 *     and this seeder's trash sweep only ever touches `_duy_othc_id` posts.
 *
 ...
 *   /wp-admin/?duy_othc_seed=1&token=<DUY_HS_SEED_TOKEN>
 *
 * SAFETY: refuses on a public production site and without the token. Remove this
 * file (and inc/data/other-countries/) before shipping the theme to production.
 *
 * @package DUY_Study
 */
...
function duy_othc_seed_run(): void {
	if ( ! isset( $_GET['duy_othc_seed'] ) ) {
		return;
	}
	...
	$report = duy_othc_seed_import();
	if ( is_string( $report ) ) {
		wp_die( esc_html( $report ) );
	}
	wp_die( '<pre>' . esc_html( duy_othc_seed_report_text( $report ) ) . '</pre>', 'Other countries seed', [ 'response' => 200 ] );
}
add_action( 'admin_init', 'duy_othc_seed_run' );

/**
 * Format a seed result array as a plain-text report.
 */
function duy_othc_seed_report_text( array $r ): string {
	return sprintf(
		"OTHER COUNTRIES SEED DONE\nRows: %d\nImported/updated: %d\nTrashed: %d\nMissing thumbnail: %d\nErrors: %d\n%s",
		$r['rows'] ?? 0,
		$r['imported'] ?? 0,
		$r['trashed'] ?? 0,
 ...
 */
function duy_othc_term_id( string $slug, string $taxonomy, string $name ): int {
 ...
 * @return array|string
 */
function duy_othc_seed_import() {
	$file = get_stylesheet_directory() . '/inc/data/other-countries/other-countries-import.json';
	if ( ! is_readable( $file ) ) {
	...
	$country_ids = [];
	foreach ( duy_othc_countries() as $slug => $name ) {
		$country_ids[ $slug ] = duy_othc_term_id( $slug, 'country', $name );
	}
	$level_ids = [
		'thpt'    => duy_othc_term_id( 'thpt', 'study_level', 'THPT' ),
		'dai-hoc' => duy_othc_term_id( 'dai-hoc', 'study_level', 'Đại học' ),
		'anh-ngu' => duy_othc_term_id( 'anh-ngu', 'study_level', 'Anh ngữ' ),
	];
	...
	foreach ( $rows as $r ) {
		...
		$keep[ $cid ] = true;
		try {
			$ex  = get_posts( [ 'post_type' => 'school', 'post_status' => 'any', 'numberposts' => 1, 'fields' => 'ids', 'meta_query' => [ [ 'key' => '_duy_othc_id', 'value' => $cid ] ] ] );
			$pid = $ex ? (int) $ex[0] : 0;
			...
			update_post_meta( $pid, '_duy_othc_id', $cid );
			update_post_meta( $pid, '_duy_seo_article', 1 );
			update_post_meta( $pid, '_duy_othc_level_slug', $level_slug );
			update_post_meta( $pid, '_duy_othc_country_slug', $country_slug );
			update_post_meta( $pid, '_duy_othc_type', (string) ( $r['institution_type'] ?? '' ) );
			...
			// Featured image = logo from the pre-uploaded /uploads/other-countries/ file.
			...
			$fpath = $updir['basedir'] . '/other-countries/' . $base;
			if ( $base && file_exists( $fpath ) ) {
				...
				if ( ! $cur_ok ) {
					foreach ( get_posts( [ 'post_type' => 'attachment', 'post_parent' => $pid, 'numberposts' => -1, 'fields' => 'ids', 'meta_key' => '_duy_othc_thumb' ] ) as $old ) {
					...
					$aid  = wp_insert_attachment(
						[
							...
							'post_status'    => 'inherit',
							'guid'           => $updir['baseurl'] . '/other-countries/' . $base,
						],
						$fpath,
						$pid,
						true
					...
					if ( ! is_wp_error( $aid ) ) {
						update_post_meta( (int) $aid, '_wp_attached_file', 'other-countries/' . $base );
						update_post_meta( (int) $aid, '_wp_attachment_image_alt', (string) ( $r['thumbnail_alt'] ?? '' ) );
						update_post_meta( (int) $aid, '_duy_othc_thumb', 1 );
	...
	// Trash imported other-countries schools no longer in the dataset.
	// Scoped strictly to `_duy_othc_id` posts — other imports are never affected.
	$trashed = 0;
	foreach ( get_posts( [ 'post_type' => 'school', 'post_status' => 'publish', 'numberposts' => -1, 'fields' => 'ids', 'meta_key' => '_duy_othc_id' ] ) as $id ) {
		$cid = (string) get_post_meta( $id, '_duy_othc_id', true );
		if ( ! isset( $keep[ $cid ] ) ) {
			wp_trash_post( $id );
			$trashed++;
		}
